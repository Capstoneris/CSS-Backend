...
public class LoginRESTResource {
    ...
    public Response login(JsonLoginInformation loginInformation) {
        ...
        if (token.equals("401")) {
            Logger.log(Service.REST, "Login failed, user " + loginInformation.username + " not found!");
            response = Response.status(401).build();
        } else if (token.equals("500")) {
            Logger.log(Service.REST, "Login failed, internal error");
            response = Response.status(500).build();
        } else {
            response = Response.ok().entity(new JsonLoginResponse(new JsonUser(0, loginInformation.username), token)).build();
            Logger.log(Service.REST, "Login successful, sending token to user " + loginInformation.username);
        }
